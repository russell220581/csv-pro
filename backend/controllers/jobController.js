...
import { validateRecipe } from "../utils/validationSchemas.js";
...
const initiateUpload = async (req, res, next) => {
    const user = req.user;
    // Use effectivePlan for admin users
    ...
    try {
        ...
        const conditions = [
            ['content-length-range', 1, PLAN_LIMITS[userPlan].fileSize],
...
const createJob = async (req, res, next) => {
    ...
    try {
        ...
        // Friendly recipe validation
        const { success, data, errors } = validateRecipe(operations);
        if (!success) {
            logger.error("Invalid job recipe", { userId: user.id, errors });
            return res
                .status(400)
                .json({ success: false, message: "Invalid recipe", errors });
        }
        const userOperations = data;
        
        // 1. Get file data for analysis (first 1000 rows for quick analysis)
        const sampleData = await fileService.getFileSample(s3Key, 1000);
        ...
        // 4. Enhanced operation processing with AGGRESSIVE DEDUPLICATION
        const allOperations = [
            ...CRITICAL_OPERATIONS,
        ...
        // LIMIT operations to prevent hanging - MAX 10 total operations
        const operationMap = new Map();
        const limitedOperations = [];

        allOperations.forEach(op => {
            const opKey = `${op.type}:${JSON.stringify(op.params || {})}`;
            if (!operationMap.has(opKey) && limitedOperations.length < 10) {
                operationMap.set(opKey, op);
                limitedOperations.push(op);
            }
        });
        ...
        console.log('Job creation with enhanced operations:', {
            ...
            uniqueOperations: uniqueOperations.length,
            duplicatesRemoved: allOperations.length - uniqueOperations.length,
            userPlan: userPlan
        });
        ...
        const newJob = await Job.create({
            ...
            inputS3Key: s3Key,
            operations: uniqueOperations, // Use deduplicated operations
        ...
        logger.info('Job created with enhanced operations', {
            jobId: newJob._id,
            userId: user.id,
            fileName: originalFileName,
        ...
        // Queue job for processing (unchanged)
