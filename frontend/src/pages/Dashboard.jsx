...
import { cleanDataInBrowser, cleanHeaders } from '@csvpro/cleaning-engine/browser';
    ...
    switch (action.type) {
        ...
        case 'ANALYSIS_COMPLETE':
            return {
                ...
                analysisReport: action.payload.analysisReport.analysisReport || action.payload.analysisReport, // FIX: Handle both old and new format
...
const Dashboard = () => {
    ...
    const processedPreview = useMemo(() => ({
        data: cleanDataInBrowser(originalPreview.data, operations),
        headers: originalPreview.headers
    }), [originalPreview, operations]);
    ...
    const handleProcessFile = useCallback(async () => {
        ...
        console.log('Operations before processing:', operations); // DEBUG
        ...
        try {
            ...
            // Remove id properties before sending to backend
            const operationsWithoutIds = operations.map(({ id, ...op }) => op);
            
            console.log('Operations after removing IDs:', operationsWithoutIds); // DEBUG
            
            const { data: createJobData } = await api.post('/jobs/create', { 
                s3Key: key, 
                originalFileName: selectedFile.name, 
                fileSize: selectedFile.size, 
                operations: operationsWithoutIds 
            });
        ...
        switch (step) {
            case 'parsing': return ( 
                <div className="card bg-base-100 shadow-xl">
                    <div className="card-body items-center p-8">
                        <Loader size="lg" text="Analyzing your file for cleaning opportunities..." />
                    </div>
                </div> 
            );
            ...
            case 'cleaning': return (
                <div className="lg:flex lg:gap-8 h-full min-h-0 lg:items-start">
                    ...
                    <main className="lg:w-3/5 xl:w-2/3 flex-grow flex flex-col gap-8 min-h-0"><MainContent analysisReport={analysisReport} headerStatus={headerStatus} applyOperation={addOperation} onApplyAll={applyAllSuggestions} processedPreview={processedPreview} onProcessFile={handleProcessFile} isProcessing={isSubmitting} operations={operations} /></main>
                </div>
            );
            default: return ( 
                <div className="card bg-base-100 shadow-xl">
                    <div className="card-body">
                        <h2 className="card-title text-2xl">Start a New Job</h2>
                        <p>Upload a CSV file to begin cleaning</p>
                        <div className="mt-4">
                            <FileDropzone onFileSelect={handleFileSelect} isProcessing={isSubmitting} selectedFile={null} />
                        </div>
                    </div>
                </div> 
            );
